...
function listSections()
{
	...
	if ($rst = $vbulletin->db->query_read($sql))
	{
		...
		while($row = $vbulletin->db->fetch_array($rst))
		{
			...
			{
				$stack[$count][] = "<span style=\"margin-" . vB_Template_Runtime::fetchStyleVar('left') . ":20px;margin-" . vB_Template_Runtime::fetchStyleVar('right') . ":0.5em;\">" . htmlspecialchars_uni($row['section']) . "</span>"  ;
			}
		}
	}
