...
*/
function print_style($styleid, $style = array(), $mastertype = -1)
{
	...
	$style['title'] = isset($style['title']) ? htmlspecialchars_uni($style['title']) : '';
	if ($styleid == -1)
	{
		$master = 'standard';
	...
	foreach ($stylevar_dfn_cache AS $stylevargroupname => $stylevargroup)
	{
		...
		foreach($stylevargroup AS $stylevar)
		{
			$xml->add_tag('stylevar', '',
				array(
					'name'       => htmlspecialchars_uni($stylevar['stylevarid']),
...
function check_template_errors($template)
{
	...
	else
	{
		...
		$oldlevel = error_reporting(E_ERROR | E_PARSE | E_CORE_ERROR | E_COMPILE_ERROR | E_USER_ERROR | E_STRICT | E_DEPRECATED);
...
function generate_style($data, $parentid, $title, $anyversion = false, $displayorder, $userselect, $version)
{
	...
	foreach ($stylevarparts AS $stylevarid => $stylevar)
	{
		...
		if($masterlist[$stylevarid])
		{
			$xml->add_tag('stylevar', '',
				array(
					'name'  => htmlspecialchars_uni($stylevarid),
