		...
		'response' => array(
			'layout' => array(
				'content' => array(
					...
					'message', 'message_plain', 'message_bbcode',
				)
			)
		)
	);
...
function api_result_prewhitelist_1(&$value)
{
	...
	if ($methodsegments[1] == 'view' AND $value['response'])
	{
		$value['response']['layout']['content']['title'] = unhtmlspecialchars($value['response']['layout']['content']['title']);

		if ($value['response']['layout']['content']['content']['contents'])
		{
			foreach ($value['response']['layout']['content']['content']['contents'] AS $k => &$v)
			{
