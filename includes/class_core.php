...
define('FILE_VERSION', '4.2.2 Alpha 1'); // this should match installsteps.php
define('SIMPLE_VERSION', '422a1'); // see vB_Datastore::check_options()
...
*
* @package	vBulletin
* @version	$Revision: 77141 $
* @date		$Date: 2013-08-27 16:56:05 -0700 (Tue, 27 Aug 2013) $
*/
class vB_Database
{
	/**
	...
	*/
	function lock_tables($tablelist)
	{
		if (!empty($tablelist) AND is_array($tablelist))
		{
			...
			// PHP 5.5+ only supports mysqli, so no pconnect again.
			if (version_compare(phpversion(), '5.5.0', '<')
				AND strtolower($this->registry->config['Database']['dbtype']) != 'mysqli' 
				AND $this->registry->config['MasterServer']['usepconnect'])
			{
				return;
			}
...
class vB_Input_Cleaner
{
	...
	*/
	function vB_Input_Cleaner(&$registry)
	{
		...
		//Deprecated/Removed as of php 5.3
...
abstract class vB_Template_Data extends vB_Template
{
	...
	public function render($suppress_html_comments = false, $final = false)
	{
		...
		if ($final)
		{
			...
			{
				return '<pre>'.htmlspecialchars_uni(var_export($this->registered, true)).'</pre>' . '<br />' . number_format((memory_get_usage() / 1024)) . 'KB';
			}
			else
			{
...
function vb_error_handler($errno, $errstr, $errfile, $errline)
{
	...
	switch ($errno)
	{
		...
		case E_WARNING:
		case E_USER_WARNING:
			/* Don't log warnings here due to the false bug reports 
			about valid warnings that we suppress, but still appear in the log
			*/
			...
			$errstr = str_replace(DIR, '....', $errstr);
			echo "<br /><b>Warning</b>: $errstr in <strong>$errfile</strong> on line <strong>$errline</strong><br />";
		break;
...
// #############################################################################
/**
* Unicode-safe version of htmlspecialchars_uni()
*
