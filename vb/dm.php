...
abstract class vB_DM
{
	...
	*/
	public function setExisting($item, $reset = true)
	{
		if ($item instanceof vB_Item)
		{
			...
			{
				throw (new vB_Exception_DM('Item class given to DM for modification is not of the DM\'s defined type \'' . htmlspecialchars_uni($this->item_class . '\'')));
			}
	...
	protected function setExistingField($fieldname, $value, $skip_undefined = false)
	{
        ...
        if ($error)	
		{
			throw (new vB_Exception_DM('Existing field \'' . htmlspecialchars_uni($fieldname) . '\' is not valid: ' . $error));
		}
	...
	*/
	public function getField($fieldname, $ignore_errors = false)
	{
		...
		{
			throw (new vB_Exception_DM('A field value for \'' . htmlspecialchars_uni($fieldname) . '\' was requested from the DM \'' . get_class($this) . '\' but the field is not defined'));
		}
	...
	*/
	public function unsetField($fieldname)
	{
		if (!isset($this->fields[$fieldname]))
		{
			throw (new vB_Exception_DM('Call to unset undefined field \'' . htmlspecialchars_uni($fieldname) . '\' in DM \'' . get_class($this) . '\''));
		}
	...
	protected function assertTable($table)
	{
		...
		{
			throw (new vB_Exception_DM('Fetching save SQL for undefined table \'' . htmlspecialchars_uni($table) . '\' in DM \'' . get_class($this) . '\''));
		}
	...
	*/
	protected function assertRequired()
	{
		foreach ($this->fields AS $fieldname => $field)
		{
			if ($field[self::VF_REQ] == self::REQ_YES AND !isset($this->set_fields[$fieldname]))
			{
				if ($this->strict)
				{
					throw (new vB_Exception_DM('Required field \'' . htmlspecialchars_uni($fieldname) . '\' has not been set'));
				}
	...
	protected function isUpdated($fieldname)
	{
		...
		if (!isset($this->fields[$fieldname]))
		{
			throw (new vB_Exception_DM('isUpdated() was called for the field \'' . htmlspecialchars_uni($fieldname) . '\' from the DM \'' . get_class($this) . '\' but the field is not defined'));
		}
