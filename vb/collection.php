...
abstract class vB_Collection extends vB_Model implements ArrayAccess, Iterator
{
	...
	protected function applyLoad($result, $load_query)
	{
		...
		if (self::QUERY_BASIC == $load_query)
		{
			...
			if ($this->paginate)
			{
				...
				$record = $this->important ? vB::$db->query_first($sql) : vB::$db->query_first_slave($sql);
	...
	public function setInfo($iteminfo, $load_flags = false)
	{
		...
		{
			$itemid = $iteminfo[$this->primary_key];
		...
		{
			throw (new vB_Exception_Model('Setting collection item iteminfo for an item that is not in the collection: \'' . htmlspecialchars_uni($iteminfo[$this->primary_key]) . '\''));
		}
	}
