...
function transferMoney($from,$to,$amount,$comment,$inhand = null,$do_log = true,$autopm = true,$taxrate = false,$output = true,$postid = 0,$more_query = false,$forceSecure = false) {
	...
	*/
	...
	$secureMode = false;
	if ($inhand !== null AND $from != 0 AND $moneyfrom > 0 //only activate secure mode if inhand is specified, not from system
		AND ($vbulletin->kbank['secure_mode'] OR $forceSecure)
	)
	{
		//secure mode
		$secureMode = true;
